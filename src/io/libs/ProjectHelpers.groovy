...
def createDb(platform, server1c, serversql, base, cfdt, isras, sqlUser, sqlPwd) {
    ...
    returnCode = utils.cmd("oscript one_script_tools/dbcreator.os ${platformLine} -server1c ${server1c} -serversql ${serversql} -base ${base} ${cfdtpath} ${israspath} -sqluser ${sqlUser} -sqlpassw ${sqlPwd} ")
    if (returnCode != 0) {
